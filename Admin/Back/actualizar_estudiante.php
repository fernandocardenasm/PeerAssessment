    ...
    if(empty($_SESSION['user'])||empty($_SESSION['logged'])||empty($_SESSION['class'])){
        ...
        if($_SESSION['class']!="admin"){
			...
			if ($json->contrasena=='CONSERVE'){
				$sql = "UPDATE estudiantetbl SET codigo=$json->codigo,apellidos='$json->apellidos',nombre='$json->nombres',usuario='$json->usuario' WHERE codigo=$json->id";
			}else{
				$sql = "UPDATE estudiantetbl SET codigo=$json->codigo,apellidos='$json->apellidos',nombre='$json->nombres',usuario='$json->usuario',contrasenha=NULL WHERE codigo=$json->id";
			}
			if (mysql_query($sql)){
