...
<body>
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                            ...
                            <ul class="nav nav-second-level">
                                ...
                                <li>
                                    <a class="active" href="admins_educativo.php">Admins. Educativos</a>
                                </li>
                            </ul>
                            <!-- /.nav-second-level -->
                        </li>                        
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Administradores educativos</h1>
					<div class="panel-body">
						<a data-toggle="modal" id="botonCrear" href="#nuevoAdminEducativo" class="btn btn-primary btn-large">
							Nuevo
						</a>
						...
						<!--Modal:Nuevo Administrador Educativo-->
							<div id="nuevoAdminEducativo" class="modal fade">
								  ...
								  <div class="modal-content"> 
									  <form id="newUserForm">
										 <div class="modal-header">
											...
											<h3>Nuevo Administrador Educativo</h3>
										 </div>
										 ...
										 <div class="modal-footer">
											<button id="confirmarNuevoAdminEducativo" switch="nuevoUsuario" type="button" class="btn btn-success">Crear</button>
						...
						<!--Fin Modal:Nuevo Administrador Educativo-->
						<!--Modal:Editar Administrador Educativo-->
							<div id="editarAdminEducativo" class="modal fade">
							   <div class="modal-dialog">   
								  <div class="modal-content"> 
									  <form id="editUserForm">
										 <div class="modal-header">
											...
											<h3>Editar Administrador Educativo</h3>
										 </div>
										 <div class="modal-body">
											<div class="form-group">
												<label>Código:</label>
										 ...
										 <div class="modal-footer">
										    <button id="confirmarEditarAdminEducativo" switch="editarProfesor" type="button" class="btn btn-success">Editar</button>
						...
						<!--Fin Modal:Editar Administrador Educativo-->
						<!--Modal:Borrar Administrador Educativo-->
							<div id="borrarAdminEducativo" class="modal fade">
							   <div class="modal-dialog">   
								  <div class="modal-content"> 
									 <div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
										...
										<h3>Borrar Administrador Educativo</h3>
										<p>¿Está seguro que quiere borrar de la base de datos a este administrador educativo? Si la acción resulta ser exitosa, no se podrá deshacer.</p>
									 </div>
									 <div class="modal-footer">
										<a href="#" data-dismiss="modal" id="confirmarBorrarAdminEducativo" class="btn btn-success">Borrar</a>
										<a href="#" data-dismiss="modal" class="btn">Cerrar</a>
									 </div>
								</div>
							   </div>
							</div>
						<!--Fin Modal:Borrar Administrador Educativo-->
	...
	$(document).ready(function(){
		...
		$("#confirmarNuevoAdminEducativo").attr("disabled", true);
		...
		$("#confirmarNuevoAdminEducativo").click(function(){
			crear(table);
		});
		...
		$("#botonEditar").click(function(){
			if (verificarSeleccion(table)==true){
				...
				alert('No ha seleccionado un administrador educativo para editar su información');
			}
		});
		
		$("#botonBorrar").click(function(){
			if (verificarSeleccion(table)==true){
				$('#borrarAdminEducativo').modal('show');				
			}else{
				alert('No ha seleccionado un administrador educativo para borrar su información');
			}	
		});
		
		$("#confirmarEditarAdminEducativo").click(function(){
			editar(table);					
		});
		
		$("#confirmarBorrarAdminEducativo").click(function(){
			borrar(table);
		});
	...
	function cargar_informacion(table){
		...
		$.ajax({
			type:"POST",
			data:{},
			url:"../Back/cargar_informacion_admins_educativo.php",
			dataType:'JSON',
			success: function(h){
				if(h.exito=="exito"){
					var i;
					...
					alert("No hay administradores educativos en la base de datos.");
				}
				else{
	...
	function crear(table){
		...
		$.ajax({
			type:"POST",
			url:"../Back/crear_admin_educativo.php",
			...
			success: function(response){
				...
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se ha agregado el(la) administrador(a) educativo(a) '+response.nombres+' '+response.apellidos+' a la base de datos de manera exitosa.');
	...
	function cargarSeleccion(table){
		$('#editarAdminEducativo').modal('show');	
	...
	//Generate JSON
		...
		$.ajax({
			type:"POST",
			url:"../Back/actualizar_admin_educativo.php",
			data: {datos: JSON.stringify(usuario)},
			dataType: "JSON",
			success: function(response){
				if(response.exito=='exito'){
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se han actualizado los datos del(la) administrador(a) educativo(a) '+response.nombres+' '+response.apellidos+' en la base de datos de manera exitosa.');
				}else if(response.exito=='fracaso'){
					alert('La consulta no se pudo realizar:\n'+response.mysql);
				}else{						
					alert("Hubo un problema con la transacción. Inténtelo más tarde por favor.");
	...
	//Generate JSON
		...
		$.ajax({
			type:"POST",
			url:"../Back/borrar_admin_educativo.php",
			...
			success: function(response){
				...
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se ha borrado el(la) administrador(a) educativo(a) '+response.nombres+' '+response.apellidos+' de la base de datos de manera exitosa.');
				}else if(response.exito=='fracaso'){
					alert('La consulta no se pudo realizar:\n'+response.mysql);
				}else{						
					alert("Hubo un problema con la transacción. Inténtelo más tarde por favor.");
