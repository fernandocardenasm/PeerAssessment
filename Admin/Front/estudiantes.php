...
<body>
            ...
            <div class="navbar-default sidebar" role="navigation">
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                            <a class="active" href="estudiantes.php"><i class="glyphicon glyphicon-user fa-fw"></i> Estudiantes</a>
                        </li>
                        <li>
            ...
            <div class="row">
                <div class="col-lg-12">
					...
					<div class="panel-body">
						<a data-toggle="modal" id="botonCrear" href="#nuevoEstudiante" class="btn btn-primary btn-large">
							Nuevo
						</a>
						...
						<!--Modal:Nuevo Estudiante-->
							<div id="nuevoEstudiante" class="modal fade">
								  ...
								  <div class="modal-content"> 
									  <form id="newUserForm">
										 ...
										 <div class="modal-footer">
											<button id="confirmarNuevoEstudiante" switch="nuevoUsuario" type="button" class="btn btn-success">Crear</button>
						...
						<!--Fin Modal:Nuevo Estudiante-->
						<!--Modal:Editar Estudiante-->
							<div id="editarEstudiante" class="modal fade">
							   <div class="modal-dialog">   
								  <div class="modal-content"> 
									  <form id="editUserForm">
										 ...
										 <div class="modal-footer">
										    <button id="confirmarEditarEstudiante" switch="editarUsuario" type="button" class="btn btn-success">Editar</button>
						...
						<!--Modal:Borrar Estudiante-->
							<div id="borrarEstudiante" class="modal fade">
							   <div class="modal-dialog">   
								  <div class="modal-content"> 
									 <div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
										...
										<p>¿Está seguro que quiere borrar de la base de datos a este estudiante? Si la acción resulta ser exitosa, no se podrá deshacer.</p>
									 </div>
									 <div class="modal-footer">
										<a href="#" data-dismiss="modal" id="confirmarBorrarEstudiante" class="btn btn-success">Borrar</a>
										<a href="#" data-dismiss="modal" class="btn">Cerrar</a>
									 </div>
								</div>
							   </div>
							</div>
						<!--Fin Modal:Borrar Estudiante-->
	...
	$(document).ready(function(){
		...
		$("#confirmarNuevoEstudiante").click(function(){
			crear(table);
		});
		...
		$("#botonEditar").click(function(){
			if (verificarSeleccion(table)==true){
				...
				alert('No ha seleccionado un estudiante para editar su información');
			}
		});
		
		$("#botonBorrar").click(function(){
			if (verificarSeleccion(table)==true){
				$('#borrarEstudiante').modal('show');				
			}else{
				alert('No ha seleccionado un estudiante para borrar su información');
			}	
		});
		
		$("#confirmarEditarEstudiante").click(function(){
			editar(table);					
		});
		
		$("#confirmarBorrarEstudiante").click(function(){
			borrar(table);
		});
	...
	function cargar_informacion(table){
		...
		$.ajax({
			type:"POST",
			data:{},
			url:"../Back/cargar_informacion_estudiantes.php",
			dataType:'JSON',
			success: function(h){
				if(h.exito=="exito"){
					var i;
	...
	function crear(table){
		...
		$.ajax({
			...
			success: function(response){
				...
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se ha agregado el estudiante '+response.nombres+' '+response.apellidos+' a la base de datos de manera exitosa.');
	...
	//Generate JSON
		...
		$.ajax({
			type:"POST",
			url:"../Back/actualizar_estudiante.php",
			data: {datos: JSON.stringify(usuario)},
			dataType: "JSON",
			success: function(response){
				if(response.exito=='exito'){
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se han actualizado los datos del estudiante '+response.nombres+' '+response.apellidos+' en la base de datos de manera exitosa.');
				}else if(response.exito=='fracaso'){
					alert('La consulta no se pudo realizar:\n'+response.mysql);
				}else{						
					alert("Hubo un problema con la transacción. Inténtelo más tarde por favor.");
	...
	//Generate JSON
		...
		$.ajax({
			type:"POST",
			url:"../Back/borrar_estudiante.php",
			...
			success: function(response){
				...
				table.$('tr.selected').removeClass('selected');
				cargar_informacion(table);
				alert('Se ha borrado el estudiante '+response.nombres+' '+response.apellidos+' de la base de datos de manera exitosa.');
				}else if(response.exito=='fracaso'){
					alert('La consulta no se pudo realizar:\n'+response.mysql);
				}else{						
					alert("Hubo un problema con la transacción. Inténtelo más tarde por favor.");
